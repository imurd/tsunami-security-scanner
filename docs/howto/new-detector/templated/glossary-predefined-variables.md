...
-   `T_CBS_DNS`: The callback server hostname only. This is used for cases where
    the full URI won't work, for example if the vulnerable software is not
    capable of making HTTP requests. It contains the callback server address
    prepended with the hashed secret (e.g.
    `8fe7d878787d65.tsunami-callback.lan`);
